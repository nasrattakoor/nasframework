...
NWindow::WindowClass NWindow::WindowClass::wndClass; // static member definition

NWindow::WindowClass::WindowClass() noexcept
{
	WNDCLASSEX wc = { 0 };
	wc.cbSize = sizeof wc;
...
const wchar_t* NWindow::WindowClass::GetName() noexcept
{
	return wndClassName;
}

HINSTANCE NWindow::WindowClass::GetInstance() noexcept
{
...
NWindow::NWindow(int x, int y, int width, int height, const wchar_t * name, HWND hWndParent) noexcept
	: 
	x(x),
	y(y),
	width(width),
	height(height),
	name(name),
	hInstance(NWindow::WindowClass::GetInstance()),
...
HWND NWindow::GetHandle()
{
	return hWnd;
}


LRESULT CALLBACK NWindow::MessageHandlerSetup(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) noexcept
{
	if (msg == WM_NCCREATE)
	{
		const CREATESTRUCT* const cs = reinterpret_cast<CREATESTRUCT*>(lParam);
		NWindow* pWnd = reinterpret_cast<NWindow*>(cs->lpCreateParams);
...
LRESULT CALLBACK NWindow::MessageHandlerRouter(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) noexcept
{
	NWindow* const pWnd = reinterpret_cast<NWindow*>(GetWindowLongPtr(hWnd, GWLP_USERDATA));
	return pWnd->HandleMsg(hWnd, msg, wParam, lParam);
}

LRESULT NWindow::HandleMsg(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) noexcept
{
	switch (msg)
	{
	//case WM_CREATE:
	...
	//	{
	//		Button_SetText(HWND(lParam), L"Greetings!");
	//	}
	//}
	/**************** END MOUSE MESSAGES *********************/
	}
